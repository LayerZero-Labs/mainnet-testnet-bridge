...
describe("NativeOFTV2: ", function () {
	...
	let owner, ownerAddressBytes32, alice, aliceAddressBytes32, lzEndpointBase, lzEndpointOther, nativeOFT, otherOFT, LZEndpointMock, NativeOFT, OFT

	before(async function () {
		[owner, alice] = await ethers.getSigners();
		ownerAddressBytes32 = ethers.utils.defaultAbiCoder.encode(["address"], [owner.address])
		aliceAddressBytes32 = ethers.utils.defaultAbiCoder.encode(["address"], [alice.address])
		LZEndpointMock = await ethers.getContractFactory("LZEndpointMock")
		NativeOFT = await ethers.getContractFactory("NativeOFTV2")
		OFT = await ethers.getContractFactory("OFTV2")
	})
	...
	beforeEach(async function () {
		...
		// 1. base chain
		// 2. other chain
		nativeOFT = await NativeOFT.deploy(name, symbol, sharedDecimals, lzEndpointBase.address)
		otherOFT = await OFT.deploy(name, symbol, sharedDecimals, lzEndpointOther.address)
	...
	it("sendFrom() - tokens from main to other chain using default", async function () {
		...
		// estimate nativeFees
		let nativeFee = (await nativeOFT.estimateSendFee(otherChainId, ownerAddressBytes32, totalAmount, false, "0x")).nativeFee

		expect(await ethers.provider.getBalance(lzEndpointBase.address)).to.be.equal(0)
		...
		let transFee = ownerBalance.sub(await ethers.provider.getBalance(owner.address)).sub(nativeFee)
		expect(await ethers.provider.getBalance(alice.address)).to.be.equal(aliceBalance.add(totalAmount))
		expect(await ethers.provider.getBalance(owner.address)).to.be.equal(ownerBalance.sub(nativeFee).sub(transFee))
	...
	it("sendFrom() - with enough native", async function () {
		...
		expect(await ethers.provider.getBalance(lzEndpointOther.address)).to.be.equal(0)
		expect(await nativeOFT.balanceOf(nativeOFT.address)).to.be.equal(totalAmount)
		expect(await nativeOFT.balanceOf(owner.address)).to.be.equal(leftOverAmount)
		expect(await otherOFT.balanceOf(owner.address)).to.be.equal(totalAmount)
	})
	...
	it("sendFrom() - from != sender with not enough native", async function () {
		...
		let messageFee = ethers.utils.parseUnits("0.5", 18)
		await nativeOFT.setUseCustomAdapterParams(false)
		await otherOFT.setUseCustomAdapterParams(false)
		await expect(
